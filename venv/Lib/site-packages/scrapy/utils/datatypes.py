...
class CaseInsensitiveDict(collections.UserDict):
    ...
    def __getitem__(self, ***REMOVED***AnyStr) -> Any:
        normalized_key = self._normkey(key)
        return super().__getitem__(self._keys[normalized_key.lower()])

    def __setitem__(self, ***REMOVED***AnyStr, value: Any) -> None:
        normalized_key = self._normkey(key)
        try:
    ...
    def __delitem__(self, ***REMOVED***AnyStr) -> None:
        normalized_key = self._normkey(key)
        stored_key = self._keys.pop(normalized_key.lower())
        super().__delitem__(stored_key)

    def __contains__(self, ***REMOVED***AnyStr) -> bool:  # type: ignore[override]
        normalized_key = self._normkey(key)
    ...
    def _normkey(self, ***REMOVED***AnyStr) -> AnyStr:
        return key
...
class LocalWeakReferencedCache(weakref.WeakKeyDictionary):
    ...
    def __getitem__(self, ***REMOVED***_KT) -> Optional[_VT]:  # type: ignore[override]
        try:
