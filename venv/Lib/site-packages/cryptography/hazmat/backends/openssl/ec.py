...
def _ecdsa_sig_sign(
    backend: Backend, private_***REMOVED***_EllipticCurvePrivateKey, data: bytes
) -> bytes:
...
def _ecdsa_sig_verify(
    backend: Backend,
    public_***REMOVED***_EllipticCurvePublicKey,
    signature: bytes,
    data: bytes,
) -> None:
...
class _EllipticCurvePrivateKey(ec.EllipticCurvePrivateKey):
    ...
    def exchange(
        self, algorithm: ec.ECDH, peer_public_***REMOVED***ec.EllipticCurvePublicKey
    ) -> bytes:
        if not (
