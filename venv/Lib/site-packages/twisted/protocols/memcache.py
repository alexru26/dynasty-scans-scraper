...
class MemCacheProtocol(LineReceiver, TimeoutMixin):
    ...
    def cmd_END(self):
        ...
        if cmd.command == b"get":
            if cmd.multiple:
                values = {***REMOVED***val[::2] for key, val in cmd.values.items()}
                cmd.success(values)
            else:
    ...
    def increment(self, key, val=1):
        ...
        @param ***REMOVED***the key to modify.
        @type ***REMOVED***L{bytes}
    ...
    def _incrdecr(self, cmd, key, val):
        ...
        if not isinstance(key, bytes):
            return fail(
                ClientError(f"Invalid type for ***REMOVED***{type(key)}, expecting bytes")
            )
    ...
    def replace(self, key, val, flags=0, expireTime=0):
        ...
        @param ***REMOVED***the key to replace.
        @type ***REMOVED***L{bytes}
    ...
    def get(self, key, withIdentifier=False):
        ...
        @param ***REMOVED***The key to retrieve.
        @type ***REMOVED***L{bytes}
    ...
    def _get(self, keys, withIdentifier, multiple):
        ...
        self.sendLine(fullcmd)
        if multiple:
            values = {***REMOVED***(0, b"", None) for key in keys}
    ...
    def delete(self, key):
        ...
        @param ***REMOVED***the key to delete.
        @type ***REMOVED***L{bytes}
